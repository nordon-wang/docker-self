...
- **登录仓库`login`/拉取镜像`pull`/推送`push`/提交镜像`commit`/给指定容器标签`tag`**
  ...
  - 若是直接 `docker pull <dockerName>`,会直接使用 `tag:latest`版本

    ```
    docker pull tomcat 
    docket pull tomcat:8 // 拉取指定版本
  ...
  docker commit 906ed2f74bc3 nordonwang/mysql:1.0 // namespace docker hub用户名
  docker images // 可以查看到 REPOSITORY 已经是 nordonwang/mysql， TAG 为 1.0
  ```
  ...
  docker push nordonwang/mysql:1.0 // 推送到远端仓库
  docker pull nordonwang/mysql:1.0 // 拉去远端仓库的docker镜像， 拉去的时候需要注意，push推送到远端的时候，同步更新到国内加速源需要时间，若失败，等会再试试， 若是private私有的仓库，需要登陆之后才能拉取，
  ```
...
- 查看所有本地镜像`images`/删除本地镜像`rmi`
  ...
  ```
  docker images // 获取需要删除的镜像id IMAGE ID
...
- 查看容器服务打印的日志`logs`/检阅容器`inspect`(更详细的容器信息，硬件/网络/版本等)/进入容器`exec`
  ...
  docker logs mysql  // 根据 NAMES
  docker logs 906ed2f74bc3 // 根据 CONTAINER ID
  docker logs -f 906ed2f74bc3 // -f 会持续打印logs
  docker inspect mysql // 镜像的mysql一些初始化配置
  ```
...
```dockerfile
docker run -it ubuntu echo '在ubuntu 环境运行' // 在ubuntu环境输出一句话
docker run -it ubuntu ls -la /  // 查看文件目录

docker run -it -d --name test ubuntu // 指定一个名为test的docker，在后台运行ubuntu，-it代表指定一个容器交互式的服务， -d 后台运行
docker exec -it test /bin/bash // 进入名为 test 的环境
ls // 查看目录文件
exit // 退出环境


docker run -p 8000:3306 -e MYSQL_ROOT_PASSWORD=123456 -itd --name mysql1 mysql  // 把docket中mysql的的3306端口，映射到宿主机上的8000端口上, -e 代表环境变量, MYSQL_ROOT_PASSWORD超级管理员密码， -itd指定为一个容器交互式的服务，并在后台运行， --name 指定名字

docker run -v ～/Downloads/:/home -itd --name test1 ubuntu   // 将宿主机下面的 ～/Downloads 目录 挂在到 镜像 /home 下面，
```
